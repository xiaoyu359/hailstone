...
var refreshItem = function(){
    ...
    var target = $('.hailstone_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if($.common.isNotEmpty(dataId)){
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .hailstone_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if($.common.isNotEmpty(panelUrl)){
	    ...
	    $('.mainContent .hailstone_iframe', topWindow).each(function() {
	        if ($(this).data('id') == panelUrl) {
	            $(this).show().siblings('.hailstone_iframe').hide();
	            return false;
            }
        });
    }
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="hailstone_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.hailstone_iframe').hide().parents('.mainContent').append(str1);
...
function activeWindow() {
    ...
    return $('.hailstone_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
